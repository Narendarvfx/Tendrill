...
class Task_Help_Assign_Modal(QDialog):
    def __init__(self,instance, *args, **kwargs):
        super(Task_Help_Assign_Modal, self).__init__()
        ...
        self.current_row = instance.main_window.ui.taskHelp_tbWidget.currentRow()
        self.task_help_details = instance.main_window.ui.taskHelp_tbWidget.item(self.current_row, 0).data(1)
        self.shot = self.task_help_details['shot']
        ...
        existing_artists = []
        man_day_cnt = 0
        filtered_artists = [x for x in self.all_artists if x['department'] == self.task_help_details['task_type']]
        ...
        self.diff_mds = self.task_help_details['bid_days']-man_day_cnt
        ...
        self.ui.sel_Compiler_chckBox.setEnabled(False)
    ...
    def insertRow(self, response):
        ...
        self.main_window.ui.team_tableWid.setItem(row_count, 2, QTableWidgetItem(str(response['bid_days'])))
        self.main_window.ui.team_tableWid.setItem(row_count, 3, QTableWidgetItem(str(response['assigned_date'])))
        self.main_window.ui.team_tableWid.setItem(row_count, 4, QTableWidgetItem(str(response['eta'])))
