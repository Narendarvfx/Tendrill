...
class CVersions():
    def __init__(self):
        super(CVersions, self).__init__()

    def create_version(self):
        api_response = api.getClientVersions(str(self.shot_details['id']))
    ...
    def update_ver_status(self, status):
        ...
        api.updateClientVersions(str(existing_ver['id']),data)

    def update_clientver_status(self,shot_id, status):
        '''
            For DataIO Update
        '''
        api_response = api.getClientVersions(str(shot_id))
        existing_ver = api_response.json()
        data = {
            'verified_by': self.main_window.employee_details['id'],
            'verified_date': datetime.datetime.now(),
