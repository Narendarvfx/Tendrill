...
class Task_Help_Shot_Details(QTreeWidget):
    def __init__(self, obj, type=None):
        super(Task_Help_Shot_Details, self).__init__()
        self.main_window = obj.main_window
        # self.progressBarValue(25)
        try:
        ...
        if type == 'task':
            ...
            status_code = obj.task_details['status']['code']
            status_color = obj.task_details['status']['color']
            bid_days = obj.task_details['bid_days']
            ...
            self.main_window.ui.taskProgress_Frame.hide()
        ...
        self.main_window.ui.clientName_lbl.setText(self.shot_details['sequence']['project']['client'])
        ...
        hold_status = ""
        if self.main_window.employee_details['role'] == "TEAM LEAD" or self.main_window.employee_details[
            'role'] == "QC" or self.main_window.employee_details['role'] == "SUPERVISOR":
            # approve_status = "QIP"
            approve_status = "IAP"
            retake_status = "IRT"
        elif self.main_window.employee_details['role'] == "HEAD QC" or self.main_window.employee_details[
        ...
        self.main_window.ui.nuke_btn.clicked.connect(lambda: self.launch_nuke())
        self.main_window.ui.nukeX_btn.clicked.connect(lambda: self.launch_nukeX())
        self.main_window.ui.sfx_btn.clicked.connect(lambda: self.launch_sfx())
        self.main_window.ui.psd_btn.clicked.connect(lambda: self.launch_psd())
        self.main_window.ui.mocha_btn.clicked.connect(lambda: self.launch_mocha())
        self.main_window.ui.shotProgress_Frame.hide()
        self.main_window.ui.add_taskPer_btn.hide()
        # self.main_window.ui.add_per_btn.clicked.connect(lambda: self.add_percentage("SHOT"))
        # self.main_window.ui.add_taskPer_btn.clicked.connect(lambda: self.add_percentage("TASK"))
        ...
        self.main_window.ui.shotBg.setGraphicsEffect(self.shadow)
        self.shadow = QGraphicsDropShadowEffect(self)
        self.shadow.setBlurRadius(50)
        self.shadow.setXOffset(0)
        self.shadow.setYOffset(0)
        self.shadow.setColor(QColor(0, 0, 0))
        self.main_window.ui.taskBg.setGraphicsEffect(self.shadow)

        # self.progressBarValue(self.shot_details['progress'])
        # self.counter = ""
        # self.counter = self.shot_details['progress']
    ...
    def shotDetailsTabClicked(self):
        ...
        if index == 0:
            ...
            Versions_Page.hqc_ver_table(self)
            pass
        elif index == 2:
            Team_Page(self)
    ...
    def task_status_update(self, status):
        ...
        if result == qm.Yes:
            response = api.updateTaskHelpArtist(str(self.task_details['id']), data)
            if response.status_code == 201:
                ...
                if status == "IP":
                    ...
                    self.main_window.ui.comp_btn.hide()
                self.main_window.ui.status_btn.setText(response.json()['status']['code'])
                self.main_window.ui.status_btn.setStyleSheet(
                    'color:"#3b3839";background-color:{};padding-left:15px;padding-right:15px;padding-top:5px;padding-bottom:5px'.format(
                        response.json()['status']['color']))
                self.shadow = QGraphicsDropShadowEffect()
                self.shadow.setBlurRadius(50)
                self.shadow.setXOffset(0)
                self.shadow.setYOffset(0)
                self.shadow.setColor(QColor(response.json()['status']['color']))
        ...
        if result == qm.Yes:
            ...
            if shot_response.status_code == 201:
                if status == "QIP":
                    ...
                    Versions.create_hqc_version(self)
                elif status == "IRT":
                    Versions.update_ver_status(self, status)
                elif status == "IAP" or status == "HRT" or status == "CRT":
                    Versions.update_ver_status(self, status)
                    if status == "IAP":
...
<div>
        ...
        </body></html>""".format(self.shot_details['sequence']['project']['client'], self.shot_details['sequence']['project']['name'],self.shot_details['sequence']['name'],self.shot_details['name'],self.shot_details['task_type'],self.main_window.employee_details['fullName'],path)
    ...
    def add_percentage(self, type):
        ...
        if modal.exec_():
            if type == "SHOT":
                data = {
                    'progress': modal.getPercentage()
                }
                res = api.update_ShotStatus(self.shot_details['id'], data)
                if res.status_code == 201:
                    self.progress = modal.getPercentage()
                    self.timer = QTimer()
